...
router.post('/', async (req, res) => {
    try {
        ...
        if (existingProject) {
            res.status(404).json({ error: 'Creator already has a proeject with this name.' });
        } else {
...
router.put('/', async (req, res) => {
    try {
        ...
        const existingProject = await Project.findOne({ creator, name: new_name });
        if (existingProject) {
            return res.status(400).json({ error: 'Project with the same creator and new name already exists' });
        }
